...
- **Precisão** (`AgentPrompts` + `AgentTools`): regras de **ação imediata** (nunca prometer busca — emitir o tool_call), **re-execução** (pedido repetido = buscar de novo, dados podem mudar), **nunca trocar o nicho/termo pedido**, informar quando não houver resultados, salvar nicho no perfil. `listar_ofertas` aceita **`q`** (busca livre em nome/anunciante/domínio/nicho/estrutura, com/sem acentos via `OfferManager::stripAccents`).
...
- **Onboarding focado em afiliação**: o greeting de conversa nova já pergunta o **nicho** com chips clicáveis (`question` + `options`) — nichos dinâmicos do swipe file (`OfferManager::topNiches`) + "Quero sugestões" + "Outro"; se o perfil já tem nicho, o greeting é personalizado ("Vi que você atua com X...") com chips de ação (`Buscar ofertas` / `Ver anúncios ativos` / `Trocar de nicho`). Ao responder o nicho: o sistema salva no perfil de forma **determinística** (`OfferManager::matchNiche` em `Agent::send`) e a diretiva de onboarding força a busca imediata (`isOnboardingReply` em `buildMessages` — só vale quando a mensagem é uma das opções da pergunta, nunca em pedidos livres). Ordem de descoberta no prompt: **nicho → experiência → orçamento → objetivo** (uma pergunta por vez, `profile_update`). `AgentPrompts::base()` tem exemplo few-shot do onboarding.
...
- **Pesquisa web** (`lib/Web/WebSearch.php`, tool `pesquisar_web`): investigação de mercado do Sócio (tendências, concorrentes, referências). Prioridade: **chave SerpApi do usuário (BYOK)** → **chave da plataforma com limite de 20 buscas/dia por usuário** (`WebSearch::PLATFORM_DAILY_LIMIT`; sistema/user 0 não consome) → **fallback gratuito** (DuckDuckGo Lite via POST → Bing HTML; Google direto bloqueia). Registro em `ad_spy_searches` com `kind='web'` e `source` (`byok|platform|fallback`) — não consome quota do plano. A regra de **INVESTIGAÇÃO COMPLETA** no prompt obriga esgotar swipe → bibliotecas → web antes de dizer "não encontrei".
