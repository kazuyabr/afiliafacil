...
- **IA**: padrão **Cloudflare Workers AI** (`CF_ACCOUNT_ID`/`CF_AI_TOKEN`/`CF_AI_MODEL`) — modelo padrão **`@cf/nvidia/nemotron-3-120b-a12b`** (3-5s/resposta, tool_calls corretos nos testes); alternativas grátis na UI: GLM 4.7 Flash (47s+) e Gemma 4 26B (89s+). + **BYOK** em `/admin/ai-settings.php` (catálogo **models.dev** via JS; suporta cloudflare/openai-compatible/anthropic/google; chave criptografada AES). **Base URL sugerida por provider**: ao trocar o provider, o campo é preenchido com o campo `api` do models.dev (ex.: LM Studio → `http://127.0.0.1:1234/v1`), com fallback local (`FALLBACK_URLS`) para providers sem `api` no catálogo (openai, groq, deepgram, elevenlabs...); valor digitado manualmente **nunca** é sobrescrito (`suggestBaseUrl` em chat/STT/TTS). **Em Docker**, URLs locais são convertidas para `host.docker.internal` automaticamente (aviso na UI via `IN_DOCKER`).
