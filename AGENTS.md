...
- **Fontes por provider** (`AdSpyManager::providerStatus`, `action=status`): Meta usa API oficial com token (BYOK/plataforma) ou biblioteca pública; Google exige SerpApi (BYOK; plataforma só p/ sistema) e extrai domínio de URLs coladas (além do fallback `<termo>.com`); TikTok usa scraping direto. **`SteelBrowser`** le `Settings:steel_api_url` (painel Admin → Configurações, prioridade) com fallback ao env `STEEL_API_URL` — fallback com JS rendering p/ Meta/TikTok quando o scraping direto é bloqueado; resultados vazios (provider ok mas sem ads) nao sao cacheados (recupera quando configurado).
