...
- **BYOK de busca** (`AdSpyKeys`, capabilities `adspy_serpapi`/`adspy_meta` em `user_ai_configs`): o usuário configura as próprias chaves em **IA → Busca de Anúncios**. O botão **Testar valida a chave digitada no campo** (sem exigir salvar) e, ao salvar uma chave nova, o provedor é **ativado automaticamente** (`enabled=1` — o toggle desmarcado confundia); se a chave existe mas está desativada, o teste avisa. Salvar sem chave (nem nova nem salva) é rejeitado com mensagem clara.
...
- **Fontes por provider** (`AdSpyManager::providerStatus`, `action=status`): **Meta = página pública da biblioteca** (`facebook.com/ads/library/?q=...&sort_data[mode]=total_impressions`) via **Steel Browser** + parse do **JSON Relay embutido** (`<script data-sjs>` → `search_results_connection` → `collated_results[]` — `parseRelayAds`); a API oficial com token é tentada primeiro, mas sem Advanced Access do app ela volta vazia/erro e a página pública assume (funciona para qualquer termo sem App Review). Google exige SerpApi (BYOK; plataforma só p/ sistema), extrai domínio de URLs coladas, fallback `<termo>.com` e **resolve termo→domínio via `WebSearch`** (até 2 candidatos, só quando a busca original vem vazia — "hasn't returned" chega como error didático); `region` do SerpApi só é aceita junto com `political_ads` (não enviar). TikTok usa scraping direto. **`SteelBrowser`** le `Settings:steel_api_url` (painel Admin → Configurações, prioridade) com fallback ao env `STEEL_API_URL` — fallback com JS rendering p/ Meta/TikTok quando o scraping direto é bloqueado; resultados vazios (provider ok mas sem ads) nao sao cacheados (recupera quando configurado).
- **Pills de status nunca mentem** (`providerStatus` → `level` + `state` + `action`): **verde** = configurado sem erro (buscou OK, buscou vazio ou ainda não buscou — `empty` ≠ falha) e também logo após **Testar chave com sucesso** (grava `health ok`); **amarelo** = falta configurar (Google sem SerpApi) OU limitação da própria fonte (TikTok bloqueado sem sessão — não é erro de config); **vermelho** = erro real que o usuário resolve (token Meta expirado) com clique direto na tela certa. `AdSpyHealth` só é limpo ao salvar quando o estado era `error` (apaga erro antigo sem perder estado bom); teste OK da chave/grava `ok`; teste OK do Steel limpa health (meta/tiktok); busca via **cache 24h também grava health**. Mensagens do TikTok mudam conforme o Steel está configurado ou não (nunca manda "configure Steel" com Steel já ativo).
- **Planos**: Teste Grátis (trial) · VSL Start · Afiliado Pro · Master Elite.
- **IA**: padrão **Cloudflare Workers AI** (`CF_ACCOUNT_ID`/`CF_AI_TOKEN`/`CF_AI_MODEL`) — modelo padrão **`@cf/nvidia/nemotron-3-120b-a12b`** (3-5s/resposta, tool_calls corretos nos testes); alternativas grátis na UI: GLM 4.7 Flash (47s+) e Gemma 4 26B (89s+). + **BYOK** em `/admin/ai-settings.php` (catálogo **models.dev** via JS; chave criptografada AES). **Base URL sugerida por provider**: campo `api` do models.dev (ex.: LM Studio → `http://127.0.0.1:1234/v1`) + fallback local (`FALLBACK_URLS`); valor manual nunca é sobrescrito (`suggestBaseUrl` em chat/STT/TTS).
- **Multi-SDK (ai-sdk)**: coluna `user_ai_configs.api_type` (`openai|anthropic|google|azure|cloudflare`, migration 31) derivada do campo `npm` do models.dev na UI (`@ai-sdk/anthropic` → anthropic, `@ai-sdk/azure` → azure, `@ai-sdk/google` → google, demais → openai) com **select manual**; `AiClient::resolveApiType()` usa o `api_type` (fallback ao provider para configs antigas). **anthropic/google respeitam `base_url`** (default os oficiais); **Azure** usa o formato de deployments (`api-key` + `api-version`); SDKs não suportados (bedrock/vertex) mostram aviso na UI. `AiClient::requestWithUrlFallback` testa **variantes de URL local** (`127.0.0.1` ↔ `localhost` ↔ `host.docker.internal`) — a mesma config funciona no host e no Docker (só troca em falha de conexão, nunca em HTTP 4xx/5xx).
- **Modelos locais (VRAM)**: providers locais (LM Studio/Ollama) podem funcionar **sem chave** (`AiClient::isLocalUrl`). A tela de IA lista os modelos **do servidor** com estado (`/api/v0/models`), carrega na VRAM (`/api/v1/models/load` — **não carrega de novo se já estiver carregado**) e descarrega (`/api/v1/models/unload`); ações `local-models`/`local-load`/`local-unload` em `admin/api/ai-settings.php`. **TTL ocioso** (`user_ai_configs.local_ttl`, default sugerido 60s) é enviado nas requisições de chat → o LM Studio descarrega sozinho após ociosidade (testado: VRAM liberada após 60s). JIT/Auto-Evict do LM Studio completam o ciclo de vida.
- **Modelos locais sem chave**: LM Studio/Ollama podem não exigir API Key — `AiClient::isLocalUrl()` detecta host local/rede privada e permite BYOK **sem chave** (`AiConfig::forUser`/`candidates` e `openaiCompatible` só enviam o header `Authorization` quando há chave; providers remotos continuam exigindo). O teste de conexão também aceita local sem chave.
...
- **Robustez da IA**: `AiClient::request` com timeout de **180s**; `AiClient::cloudflare` normaliza `result.response`/`choices[0].message.content` (inclusive conteúdo em array de blocos). **`max_tokens: 2048`** é enviado em todas as chamadas (openai/azure/google/cloudflare) — sem isso modelos locais truncavam o JSON no meio. O **comentário de resultado** usa `AgentPrompts::comment()` (prompt CURTO ~800 chars com few-shot do swipe vazio). `handleToolCall` é reutilizado pelo `handleResponse` e pelo `commentOnResult` com profundidade máxima 3 (anti-loop). Regra de **pedido explícito**: se o usuário pedir uma ação específica, a IA executa exatamente o pedido — a busca de ofertas do onboarding vale só quando ele apenas informa o nicho.
- **Parser de JSON tolerante** (`lib/Json.php` + `Agent::parseResponse`): remove cercas de código, repara quebras de linha literais, **fecha estruturas abertas de JSON truncado** (`closeOpenStructures`) e remove vírgulas sobrando. O `Agent::parseResponse` extrai do primeiro `{` até o último `}` **ou até o fim** quando truncado (antes o regex exigia `}` e devolvia null → o JSON cru vazava no chat).
...
- **Política de cotas por segmento** (`AiConfig::candidates`): ordem **BYOK do usuário → chave dedicada do segmento → plataforma → alternativa**. Chaves dedicadas opcionais no `.env`: **`CF_AI_TOKEN_ADMIN`/`CF_ACCOUNT_ID_ADMIN`** (plano premium — o admin não depende da cota compartilhada dos clientes) e **`CF_AI_TOKEN_TRIAL`/`CF_ACCOUNT_ID_TRIAL`** (isola o consumo do trial). `CF_AI_TOKEN_2`/`CF_ACCOUNT_ID_2` somam cota extra como alternativa.
- **Monitoramento de uso**: `ai_usage_daily` (migration 32) conta as requisições de IA por usuário/dia (`AiClient::setUsageUser` + `trackUsage` no sucesso; `usageToday`). Falhas de cota gravam `ai_quota_error_at` em settings. A tela de IA mostra o **painel de cota**: requisições hoje, status da plataforma (OK/esgotada + aviso para configurar chave alternativa/upgrade) e quais chaves estão configuradas (plataforma/alternativa/admin/trial/BYOK) — endpoint `action=usage`.
...
- **BYOK ilimitado**: quando o usuário tem chave própria ativa (`user_ai_configs` capability `chat`/`stt`/`tts`), as quotas de IA **não se aplicam** — o limite passa a ser o da própria chave do cliente (não controlamos planos de terceiros). A quota do plano vale apenas para o provider da plataforma (Cloudflare).
  ...
  - UX: pills mostram "BYOK — sem limite"; ao esgotar a cota da plataforma, a mensagem orienta configurar a chave própria (`/admin/ai-settings.php` — âncoras `#chat`/`#stt`/`#tts`/`#adspy` abrem a aba certa) em vez de só bloquear.
  - **Alertas clicáveis**: erros de chave/token no AdSpy carregam link "Configurar agora" → `/admin/ai-settings.php#adspy`; os pills de status abrem a aba certa quando o provider precisa de configuração.
