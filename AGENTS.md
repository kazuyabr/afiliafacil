...
- **Fontes por provider** (`AdSpyManager::providerStatus`, `action=status`): Meta usa API oficial com token (BYOK/plataforma) ou biblioteca pública; Google exige SerpApi (BYOK; plataforma só p/ sistema) e extrai domínio de URLs coladas (além do fallback `<termo>.com`); TikTok usa scraping direto. **`SteelBrowser`** (`STEEL_API_URL`, opcional) serve de fallback com JS rendering p/ Meta/TikTok quando o scraping direto é bloqueado — sem ele, nada muda.
