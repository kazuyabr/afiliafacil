...
- **Pills de status nunca mentem** (`providerStatus` → `level` + `state` + `action`): **verde** = última resposta OK (inclusive `empty` — respondeu sem anúncios ≠ falha, ex.: busca por domínio); **amarelo** = falta configurar OU limitação da própria fonte (TikTok bloqueado sem sessão — não é erro de config); **vermelho** = erro real que o usuário resolve (token Meta expirado) com clique direto na tela certa; **cinza** = nunca buscou. Health (`AdSpyHealth`) é **limpo** ao salvar/testar chave (`ai-settings`) e no teste OK do Steel — erro antigo nunca sobrevive a uma correção; busca via **cache 24h também grava health**. Mensagens do TikTok mudam conforme o Steel está configurado ou não (nunca manda "configure Steel" com Steel já ativo).
