...
class TtsClient
{
    ...
    public static function quotaMessage(string $tab = 'Narração'): string
    {
    ...
    private static function cloudflare(string $text, array $config, string $voice): array
    {
        ...
        if ($response === null) {
            if (self::isQuotaError()) {
                ...
                return ['success' => false, 'error' => self::quotaMessage('Narração'), 'quota_exceeded' => true, 'provider' => 'cloudflare'];
            }
